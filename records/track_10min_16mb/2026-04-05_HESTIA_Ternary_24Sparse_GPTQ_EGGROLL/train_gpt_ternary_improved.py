#!/usr/bin/env python3
"Improved ternary training script. Fork of Ciprian-Florin Ifrim's submission with: leaky_relu2, FA3/SDPA fallback, 2048 seq training."
...
# FA3 on Hopper (CC >= 9.0), SDPA fallback on older GPUs or CPU
...
class CausalSelfAttention(nn.Module):
    ...
    def _xsa_efficient(self, y: Tensor, v: Tensor) -> Tensor:
        """XSA: subtract self-value projection via GQA-aware reshape. Zero parameters."""
        B, T, H, D = y.shape
        Hkv = v.size(-2)
        group = H // Hkv
...
class Block(nn.Module):
    ...
    def forward(self, x: Tensor, x0: Tensor) -> Tensor:
        ...
        n = self.attn_norm(x)
        x = x + self.attn_scale.to(dtype=x.dtype) * self.attn(n * s if s != 1.0 else n)
        mn = self.mlp_norm(x)
        x = x + self.mlp_scale.to(dtype=x.dtype) * self.mlp(mn * s if s != 1.0 else mn)
...
class GPT(nn.Module):
    def __init__(self, vocab_size, num_layers, model_dim, num_heads, num_kv_heads, mlp_mult,
        ...
        # XSA: enable on last N layers (zero parameter cost)
        if xsa_last_n > 0:
...
def find_temp(args, base_model, rank, world_size, device, grad_accum_steps,
    ...
    for t in [0.85, 0.88, 0.90, 0.92, 0.95, 1.00, 1.05]:
...
def main() -> None:
    ...
    while True:
        ...
        # Gradient clipping (SOTA uses 0.3)
